...
import {openBuyPage, openRegisterPage, openHelpPage} from './js/actions'
import langUtils from './js/utils/lang'

const getQuickHelpHtml = () => {
  let browserLang = langUtils.getBrowserLanguage()
    ...
    if (browserLang === 'en' || langHello === undefined) {
      var zero = `'t Bonjour' = Hello`
      var one = `'t fr Hello' = Bonjour`
      var two = `'t en Bonjour' = Hello`
    } else {
      var zero = `'t ${englishHello}' = ${langHello}`
      var one = `'t en ${langHello}' = ${englishHello}`
      var two = `'t ${browserLang} ${englishHello}' = ${langHello}`
    }

  let html = `
  <b>Memo</b>: ${zero}, ${one}, ${two}
  `
...
carousel.slideCount  = carousel.innerElements.length;
...
buy.addEventListener('click', () => openBuyPage())
register.addEventListener('click', () => openRegisterPage())
end.addEventListener('click', () => carousel.goTo(0))
...
const checkButtons = () => {
  ...
  if (carousel.currentSlide >= carousel.slideCount - 1) {
...
const onCarouselChange = () => {
  ...
  let video = videos[carousel.currentSlide - 1]
    if (video) {
      resetAllVideos()
      let sources = video.querySelectorAll('source')
      sources.forEach(source => {
        let sourceFile = source.getAttribute('data-src');
...
const showLegends = (video) => {
  ...
  if (video) {
    ...
    contentElement.innerHTML = getQuickHelpHtml()
  }
}

let buttons = document.querySelectorAll('.video-trigger')

buttons.forEach(button => {

  let videoId = button.getAttribute('videoId')
  let video = document.getElementById(videoId)
  button.innerHTML = video.getAttribute('data-title')

  button.addEventListener('click', ( event ) => {

    let index = Array.from(videos).indexOf(video) + 1

    carousel.goTo(index, () => {
      onCarouselChange(video)
    })
