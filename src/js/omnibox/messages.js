...
const messages = {
  ...
  getFromSuggestDescription(languageName, languageSlice, langName, langSlice, aliasName, aliasSlice) {
    return `<match><dim>?</dim></match>${space.repeat(2)}Translate from <url><match>${languageName.slice(0, languageSlice.length)}</match>${languageName.slice(languageSlice.length, languageName.length)}</url> <dim>(<match>${!aliasSlice ? langName.slice(0, langSlice.length) : ''}</match>${!aliasSlice ? langName.slice(langSlice.length, langName.length) : ''}<match>${aliasSlice ? aliasName.slice(0, aliasSlice.length) : ''}</match>${aliasSlice ? aliasName.slice(aliasSlice.length, aliasName.length) : ''})</dim>`
  },
